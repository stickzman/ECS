export default {
    init(ecs) {
        ...
        ecs.on("BoundingBox_Removed", ev => {
            for (const entity of this.movable.tuples) {
    ...
    fixedUpdate(ecs, deltaTime) {
        for (let i = 0; i < this.movable.tuples.length; i++) {
            const moveEntity = this.movable.tuples[i]
            ...
            for (let j = 0; j < this.stationary.tuples.length; j++) {
                const stayEntity = this.stationary.tuples[j]
            ...
            for (let j = 0; j < this.movable.tuples.length; j++) {
                const other = this.movable.tuples[j]
