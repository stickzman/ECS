export default class InputSystem {
    constructor(ecs) {
        this.ecs = ecs
        this.ecs.globals.inputActions = {}
        this.ecs.globals.input = new InputState()
        this.ecs.globals.inputStream = new InputStream()
        this.ecs.globals.bindings = new KeyBindings()
    ...
    init(ecs) {
        ...
        window.addEventListener("keydown", e => {
            let action = ecs.globals.bindings.getAction(e.key.toLowerCase())
            if (action) ecs.globals.inputStream.addInput(action, true)
        })
        window.addEventListener("keyup", e => {
            let action = ecs.globals.bindings.getAction(e.key.toLowerCase())
            if (action) ecs.globals.inputStream.addInput(action, false)
        })
    }

    update(ecs) {
        const state = ecs.globals.input.state
        const inputStream = ecs.globals.inputStream

        // Reset inputState for new frame
        ecs.globals.input.reset()
    ...
    addKeyBindings(bindings) {
        ...
        for (const action in bindings) {
            ...
            this.ecs.globals.inputActions[action] = action
            this.ecs.globals.input.addAction(action)
            this.ecs.globals.bindings.addBinding(action, key)
        }
    }
}
